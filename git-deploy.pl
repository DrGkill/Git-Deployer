...

{
	...
	my $project_status;
        ...
        if (!readdir DIR){
        	# No ! I create it.
		my $git_init_cmd = "su - $sysuser -c 'cd $local_path && $git clone --depth=$depth -b $branch $git_url $local_path'";
		...
		my $stash  = `su - $sysuser -c 'cd $local_path && $git stash'`;

		# Now, update the project.
		my $status = `su - $sysuser -c 'cd $local_path && $git pull'`;
                chomp($status);
